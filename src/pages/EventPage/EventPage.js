  ...
  if (localStorage.getItem("token")) {
    ...
    if (user?._id === event.userId) {
      ...
      deleteButton.addEventListener("click", async () => {
        const deleteRes = await fetch("http://localhost:3000/api/events/" + event._id, {
          method: "DELETE",
          headers: {
    ...
    attendeesButton.addEventListener("click", async () => {
      const attendeesRes = await fetch("http://localhost:3000/api/attendees/" + event._id, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json"
        },
    ...
    joinButton.addEventListener("click", async () => {
      ...
      let pContent;
      try {
        const res = await fetch("http://localhost:3000/api/user/attendees/" + event._id, {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json"
